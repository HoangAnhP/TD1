...
double computeSampleVariance (double values[], int length)
{
    ...
    variance = length * variance / (length - 1);
    return variance;
}
...
double computeCovariance(double values1[], double values2[], int length)
{
    ...
    double s = 0;
    for(int i = 0; i < length; i++){
        s += (values1[i] - mean1)*(values2[i] - mean2);
    }
    s = s / (length - 1);
    return s;
}
...
double computeCorrelation(double values1[], double values2[], int length)
{
    ...
    double sigma1 = computeSampleStandardDeviation(values1, length);
    double sigma2 = computeSampleStandardDeviation(values2, length);
    
    double s = cov / (sigma1 * sigma2);
    return s;
}
